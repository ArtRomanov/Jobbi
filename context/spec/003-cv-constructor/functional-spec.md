...
- **As a** user, **I want to** see all my CVs in one place, **so that** I can manage and organize my CV versions.
  - **Acceptance Criteria:**
    - [ ] A "CVs" page is accessible from the main navigation bar.
    - [ ] The page lists all CVs with: name, last updated date, and a count of linked applications.
    - [ ] A "Create New CV" button is visible at the top of the page.
    - [ ] Each CV row has actions: Edit, Duplicate, Download PDF, Delete.
    - [ ] Deleting a CV shows a confirmation dialog: "Are you sure you want to delete this CV? Applications linked to it will be unlinked." On confirm, the CV is deleted.
...
- **As a** user, **I want to** create and edit a CV with structured sections, **so that** I can build a well-organized CV within Jobbi.
  - **Acceptance Criteria:**
    - [ ] The CV editor is a dedicated page accessible from "Create New CV" or by editing an existing CV.
    - [ ] The CV has a required name field (e.g., "Base CV", "Frontend CV") at the top.
    - [ ] The CV has the following sections, each with a free-text content area:
    ...
    - [ ] Work Experience and Education sections support adding and removing multiple entries via "Add" and "Remove" buttons.
    - [ ] Changes are saved with a "Save" button. On success, show a toast: "CV saved."
    - [ ] A "Back to CVs" link returns to the CV list page.
...
- **As a** user, **I want to** quickly duplicate a CV from within an application's detail panel, **so that** I can tailor it for that specific role without affecting my base CV.
  - **Acceptance Criteria:**
    - [ ] The application detail side panel shows a "CV" section displaying the currently linked CV name (or "No CV linked").
    - [ ] A "Link CV" dropdown allows selecting an existing CV to link to this application.
    - [ ] A "Duplicate & Customize" button next to the dropdown creates an exact copy of the selected CV, names it "[Original Name] — [Company] [Role]", links it to this application, and opens the CV editor for customization.
    - [ ] If no CV is selected, "Duplicate & Customize" is disabled.
    - [ ] Unlinking a CV (selecting "None" in the dropdown) removes the link but does not delete the CV.

### 2.4 PDF Export

- **As a** user, **I want to** download my CV as a PDF, **so that** I can attach it to job applications outside of Jobbi.
  - **Acceptance Criteria:**
    - [ ] A "Download PDF" button is available on the CV editor page and in the CV list row actions.
    - [ ] Clicking it generates a formatted PDF containing all CV sections with their content.
    - [ ] The PDF uses a clean, professional layout with section headings, proper spacing, and readable fonts.
    - [ ] The PDF filename is the CV name (e.g., "Frontend_CV.pdf").
