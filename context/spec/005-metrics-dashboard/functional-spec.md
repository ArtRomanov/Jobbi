...
- **As a** user, **I want to** see all my key job search metrics on one page, **so that** I can quickly assess my progress and adjust strategy.
  - **Acceptance Criteria:**
    - [ ] A "Metrics" link in the main navigation bar leads to a dedicated metrics page.
    - [ ] The page has three sections: KPI cards (top), Pipeline overview (middle), Trend chart (bottom).
    - [ ] A date range selector at the top of the page offers preset ranges: "Last 7 days", "Last 30 days", "Last 90 days", "All time". Default: "Last 30 days".
    - [ ] Changing the date range updates all metrics on the page (KPI cards, pipeline, trend).
    - [ ] The page shows a loading state while metrics are being fetched.
    - [ ] If there are no applications in the selected range, an empty state message is shown.

### 2.2 KPI Cards

- **As a** user, **I want to** see headline metrics at a glance, **so that** I instantly know my key numbers.
  - **Acceptance Criteria:**
    - [ ] Four KPI cards are displayed in a horizontal row at the top of the page:
    ...
    - [ ] Each card shows the label and the numeric value (or percentage for Response Rate).
    - [ ] Cards have visual emphasis (large numbers, clean layout).
...
- **As a** user, **I want to** see how my applications are distributed across stages, **so that** I can identify bottlenecks in my pipeline.
  - **Acceptance Criteria:**
    - [ ] A pipeline section shows the count of applications in each status: Researching, Applied, Interview, Offer, Rejected, Withdrawn.
    - [ ] Each status is displayed as a bar or column with the count and the status name.
    - [ ] Each status uses its color from the theme (matching the kanban board status colors).
    - [ ] Status counts respect the selected date range (count of applications created in that range).
    - [ ] Empty statuses (count = 0) are still shown for completeness.
...
- **As a** user, **I want to** see my application activity over time, **so that** I can understand whether my job search velocity is increasing or decreasing.
  - **Acceptance Criteria:**
    - [ ] A line chart shows the number of applications created per day (or per week for longer ranges) over the selected date range.
    - [ ] The X-axis shows dates and the Y-axis shows application count.
    - [ ] The chart auto-adjusts granularity: daily for "Last 7 days" and "Last 30 days", weekly for "Last 90 days", and weekly or monthly for "All time".
    - [ ] Hovering over a point shows the exact date and count (tooltip).
    - [ ] If there is no data in the range, a message like "No applications in this period" is shown instead of an empty chart.
